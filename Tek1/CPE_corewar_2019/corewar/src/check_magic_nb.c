...
int *base_hexadecima_to_int(char *str, int k, int *tab)
{
    char c;
    int i = 0;
...
int *get_magic_int(int *tab)
{
    ...
    tab = malloc(sizeof(int) * (my_strlen(magic_nb) / 2 + 1));
    for (; magic_nb[i] != '\0'; i++) {
        new_nb = malloc(sizeof(char) * 3);
        if (i % 2 == 0) {
            new_nb = separe_magic_nb(new_nb, magic_nb, i);
            base_hexadecima_to_int(new_nb, cpt, tab);
            cpt++;
        }
        free(new_nb);
    }
...
void check_magic_nb(int *tab)
{
    ...
    check = get_magic_int(check);
    for (int i = 0; tab[i] != -1; i++)
        if (tab[i] != check[i])
            my_putstr_error("Error : Bad magic number.");
    return;
}